...
def generateLandmarks(startLocation):
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are in " + startLocation + " as latitude and longitude coordinates. Strictly provide 5 very nearby landmarks/points of interest/cool locations that are in the city of the coordinates in this specific format and make sure its only this format because we need to parse it like that, Have a decently detailed historical and interesting background like a paragraph : (name: description: latitude: longitude). for example it should be (Berkshire Theatre Group: performing arts venue known for its great performances: 42.36: -73.29) and seperate each entry by @, like this: (name: description: latitude: longitude)@(name: description: latitude: longitude), etc."},
                {"role": "user", "content": "Generate 5 landmarks very near the location coordinates, these should be in the same city, like if coordinates are in troy ny have it only be in troy ny do not have albany  " + startLocation}
            ]
        )
